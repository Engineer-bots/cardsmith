...
import { BotModule, CoreContext } from "../module.js";
...
async function fetchPreview(url: string, logger: CoreContext["logger"]) {
  ...
  register: (context: CoreContext) => {
    ...
    client.on("messageCreate", safeEventHandler(logger, "dcEmbed:messageCreate", async (message) => {
      ...
      if (content.split(/\s+/).length !== 1) {
        logger.debug({ content }, "DC link ignored: not standalone");
        return;
      }
